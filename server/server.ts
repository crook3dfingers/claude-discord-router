...
// --- Per-session channel routing ---
// When multiple Claude Code sessions share one bot, each session filters
// inbound messages to only deliver ones matching its project's channels.
// Config lives in ~/.claude/channels/discord/routing.json.
const ROUTING_FILE = join(STATE_DIR, 'routing.json')
const PROJECT_DIR = process.env.CLAUDE_PROJECT_DIR ?? null

type ProjectRouting = { channels: string[]; dm?: boolean }
type RoutingConfig = Record<string, ProjectRouting>

function loadRouting(): { channels: Set<string>; dm: boolean } | null {
  if (!PROJECT_DIR) return null
  try {
    const raw = readFileSync(ROUTING_FILE, 'utf8')
    const config: RoutingConfig = JSON.parse(raw)
    const entry = config[PROJECT_DIR]
    ...
    return null // file missing or malformed → no filtering
  }
}
...
if (PROJECT_DIR) {
  ...
  process.stderr.write(
    r
      ? `discord channel: routing → ${r.channels.size} channel(s), dm=${r.dm} [${PROJECT_DIR}]\n`
      : `discord channel: no routing for ${PROJECT_DIR}, delivering all\n`,
  )
}
...
async function handleInbound(msg: Message): Promise<void> {
  // Per-session channel routing — drop messages not targeting this project.
  const routing = loadRouting()
  if (routing) {
    const isDM = msg.channel.type === ChannelType.DM
    if (isDM) {
      if (!routing.dm) return // another session owns DMs
    } else {
      const channelId = msg.channel.isThread()
        ? msg.channel.parentId ?? msg.channelId
        : msg.channelId
      if (!routing.channels.has(channelId)) return
    }
  }
